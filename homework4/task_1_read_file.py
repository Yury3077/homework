...
def read_magic_number(path: str) -> bool:
    ...
    Open file and read 1 line
        param: path to file
        return: true if first line is integer from 1 to 3, false in other number
    and return ValueError if the 1st line not int
    """
    with open(path, encoding="utf-8") as fi:
        first_line = fi.readline()
        if isinstance(int(first_line), int):
            num = int(first_line)
            if 1 <= num <= 3:
                return True
            else:
            ...
            raise ValueError("Value in the first line is not appropriate")
